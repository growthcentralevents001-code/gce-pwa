...
| **Enterprise core client workflow** (requirement → review → project) | **COMPLETE_WITH_P2** | `lib/architecture/enterprise/lifecycle.ts`; `/enterprise/intake`; `/enterprise/opportunities/[id]`; `/enterprise-expert/opportunities/[id]` | Client submit; expert review/qualify/assign; proposal/quote UI; project duplicate guard | `tests/unit/enterprise-requirement-timeline.test.ts`; `tests/unit/enterprise-lifecycle-access.test.ts`; `tests/e2e/enterprise/client-requirement.spec.ts`; prior cosign/milestones/idor | Enterprise BDP product **out of scope**; documents/notifications **P2**; legacy `/enterprise/signup` **P2 retire** |
