...
| **Marketplace revenue allocation** (80/10/10 vs 80/0/20) | **COMPLETE_WITH_P2** | `lib/architecture/marketplace/allocation.ts`; `calculateMarketplaceSplit`; `marketplace_revenue_entitlements`; `GET /api/venue/allocations` | Booking confirm → `allocateMarketplaceBookingRevenue`; cancel → `reverseMarketplaceBookingAllocation`; claim ≠ revenue (`92b1e59`) | `tests/unit/marketplace-allocation.test.ts`; `tests/unit/phase7-marketplace.test.ts`; `tests/e2e/venue/venue-allocations-auth.spec.ts` | Refund % economics **BLOCKED** (OD-001/006); Phase 9 stakeholder bridge **P2**; settlement **INACTIVE** |
