...
| **Enterprise BDP opportunity & operating layer** | **COMPLETE_WITH_P2** | Phase 8 spine + delta: `/enterprise-bdp`; `/enterprise-bdp/apply`; `/ops/enterprise/packs`; `lib/architecture/enterprise/{operations,lifecycle,permissions,constants}`; `enterprise_bdp_packs`; `enterprise_client_attributions`; `enterprise_revenue_entitlements`; `gce_claim_revenue_component` | Public opportunity; apply/terms/payment/approval; Ops pack queue; BDP corporate lead + attribution propose; handoff to Enterprise Core; read-only project view; 25% of eligible platform commission via `calculateEnterpriseEntitlement`; idempotent `earning_event_key` | `tests/unit/enterprise-bdp-delta.test.ts`; `tests/unit/phase8-enterprise.test.ts`; Phase 8 SQL harness | Online pack checkout **INACTIVE**; settlement/payout **INACTIVE**; finance recovery UI **P2**; documents/notifications **P2** |
