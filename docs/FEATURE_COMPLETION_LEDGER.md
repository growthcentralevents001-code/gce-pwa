...
| **Marketplace revenue allocation** (80/10/10 vs 80/0/20) | **COMPLETE_WITH_P2** | `lib/architecture/marketplace/allocation.ts`; `calculateMarketplaceSplit`; `marketplace_revenue_entitlements`; Finance read via `loadFinanceBundle` | Booking confirm → allocate; cancel → `on_hold` (not reversed until Finance approval); claim ≠ revenue (`92b1e59`+fix) | `tests/unit/marketplace-allocation.test.ts`; `tests/e2e/venue/venue-allocations-auth.spec.ts` | Refund % economics **BLOCKED** (OD-001/006); manual Phase 9 post **by design**; settlement **INACTIVE** |
| **Marketplace customer & venue operational flow** (booking/claim → credential → check-in/visit/redemption) | **COMPLETE_WITH_P2** | Phase 7/11 CX + credentials; `/customer/bookings|tickets|claims`; `/venue/check-in|redemptions|bookings`; `gce_marketplace_*` RPCs | Sandbox confirm + encrypted credentials; venue scope guards; redeem token validation fix (`20260904200000`); booking timeline | `tests/e2e/customer/booking-ticket.spec.ts`; `tests/e2e/venue/checkin.spec.ts`; `tests/e2e/offers/redemption.spec.ts`; `tests/e2e/offers/visit-redemption.spec.ts`; `tests/unit/booking-timeline.test.ts` | Live ticket payments **INACTIVE**; venue QR scanner **P2**; rank **INACTIVE** |
...
| **Enterprise core client workflow** (requirement → review → project) | **COMPLETE_WITH_P2** | `lib/architecture/enterprise/lifecycle.ts`; `/enterprise/intake`; `/enterprise/opportunities/[id]`; `/enterprise-expert/opportunities/[id]` | Client submit; expert review/qualify/assign; proposal/quote UI; project duplicate guard (app + DB); legacy `/enterprise/signup` → `/enterprise/intake`; `enterprise_applications` client insert blocked | `tests/unit/enterprise-requirement-timeline.test.ts`; `tests/unit/enterprise-lifecycle-access.test.ts`; `tests/e2e/enterprise/client-requirement.spec.ts`; `tests/e2e/legacy-redirects.spec.ts`; `tests/integration/sql/phase8_enterprise_concurrency.sql` | documents/notifications **P2** |
| **Enterprise BDP opportunity & operating layer** | **COMPLETE_WITH_P2** | Phase 8 spine + delta: `/enterprise-bdp`; `/enterprise-bdp/apply`; `/ops/enterprise/packs`; `lib/architecture/enterprise/{operations,lifecycle,permissions,constants}`; `enterprise_bdp_packs`; `enterprise_client_attributions`; `enterprise_revenue_entitlements`; `gce_claim_revenue_component` | Public opportunity; apply/terms/payment/approval; Ops pack queue; BDP corporate lead + attribution propose; handoff to Enterprise Core; read-only project view; 25% of eligible platform commission via `calculateEnterpriseEntitlement`; idempotent `earning_event_key`; **commercial authority:** pack fees FD-026 §21–24 + FD-039 §803; platform commission 20% FD-026 §52 + FD-029 §39 (not Marketplace); activation requires payment evidence | `tests/unit/enterprise-bdp-delta.test.ts`; `tests/unit/phase8-enterprise.test.ts`; Phase 8 SQL harness | Online pack checkout **INACTIVE**; settlement/payout **INACTIVE**; finance recovery UI **P2**; reduced platform commission 15–19% approval matrix **P2**; Vendor Opportunity Fee **BLOCKED** (FD-026 unresolved %) |
...
| 2026-09-04 | Financial consistency gate: cancel → `on_hold` (not auto-reversed); Finance visibility for `marketplace_revenue_entitlements`. |
| 2026-09-04 | Marketplace customer/venue operational flow recorded — redeem token validation fix, booking timeline, venue ticket check-in visibility. |
| 2026-09-05 | Enterprise core client workflow recorded — client intake, lifecycle transitions, expert workflow UI, requirement timeline. |
| 2026-09-05 | Enterprise Core integrity gate — legacy `/enterprise/signup` retired to `/enterprise/intake`; `enterprise_applications` client writes blocked; DB partial unique index `uq_enterprise_project_one_active_per_opportunity`. |
| 2026-09-05 | Enterprise BDP opportunity & operating layer — public `/enterprise-bdp`, apply lifecycle, Ops pack queue, attribution propose RBAC, corporate lead, Core handoff, read-only project visibility, entitlement idempotency. |
| 2026-09-05 | Enterprise BDP commercial authority gate — pack fees traced to FD-026/FD-039; platform commission 20% traced to FD-026 §52 / FD-029 §39; backend activation hardened to require payment evidence. |
