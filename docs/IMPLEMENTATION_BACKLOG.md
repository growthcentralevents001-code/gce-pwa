...
| Enterprise | Componentised entitlement boundary; **no double commission** | Settlement + commission | FD-038/037 | 8 / 9 | P0 | Critical | Same rupee component cannot pay two vertical commissions | Cross-vertical recon tests | **Done on gce-dev** |
...
| Finance | Ledger principles (internal wallet; cash-out inactive) | ADR-007 | FD-020/039 | 9 | P0 | Critical | Ledgers auditable; cash-out flag off | Ledger invariant tests | **Done on gce-dev** |
| Finance | Commission Engine entitlement states | `SM_Commission` | FD-029 | 9 | P0 | Critical | States versioned; self-approval impossible | Commission SM + SoD tests | **Done on gce-dev** |
...
| Admin Ops | Approval queues (membership, listings, Enterprise quotes, KYC, offline pay) | RBAC + audit | FD-036/037/038/039 | 13 | P1 | High | Queues enforce SoD; Platform final where required | Queue SoD tests | Low | **DONE** Phase 13 `ops_approval_queue` + SoD |
| Admin Ops | Exception / dispute / financial hold queues | Settlement | FD-021/029 | 13 | P1 | High | Holds block payout; overrides audited | Hold/override tests | Low | **DONE** Phase 13 exceptions/cases/overrides; holds via Phase 12 |
| Admin Ops | Support playbooks + escalation (Operational Recommendation SLAs) | Notifications | Phase 13 | 13 | P2 | Low | Playbooks linked; no invented commercial rates | Ops checklist review | Low | **DONE** foundation — macros/SOP copy still Ops polish |
| QA & migration | RLS + finance reconciliation test packs | Phases 4–9 | FD-032 | 14 | P0 | Critical | Critical policies and money paths covered pre-pilot | Automated suites in CI | High | **DONE Phase 14A** (backend SQL/unit); product E2E → 14B |
| QA & migration | Legacy data migration rehearsal (roles, statuses) | Taxonomy + SMs | FD-032 | 14 | P0 | High | Dry-run report; no silent historical rewrite | Rehearsal sign-off | High | **DONE Phase 14A posture** — clean rebuild + quarantine; bulk historical still ops |
| QA & migration | Non-prod MoR/payment config matrix | Flags + PSP sandbox | OD-001 | 14 | P0 | High | Sandbox paths tested without production claims | Config matrix tests | Low | **PARTIAL 14A** flag matrix audited OFF; full PSP matrix → 14B/15 |
| QA & migration | Phase 14B full-product E2E/UAT | Final Website/PWA | Phase 14A | 14 | P0 | High | Browser/responsive/a11y/UAT after redesign | Playwright + UAT packs | Low | **DEFERRED** until frontend redevelopment |
| Frontend | Final GCE Website/PWA gap audit + redevelopment plan | Phase 14A backend ready | FD-035/039 + MASTER.md | FE | P0 | High | Page inventory, IA, contracts, batches 0–10, tooling plan | Planning docs in `docs/frontend/` | Low | **DONE planning** — redevelopment not started |
