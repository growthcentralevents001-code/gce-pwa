...
describeDb("Phase 7 Marketplace concurrency (local Postgres)", () => {
  it("enforces splits, venue caps, offer gates, redemption/QR replay", () => {
    execFileSync(
      PSQL,
      [
        "-h",
    ...
    const out = execFileSync(
      ...
      [
        ...
        path.join(
          process.cwd(),
          "tests/integration/sql/phase7_marketplace_concurrency.sql"
        ),
      ],
      { encoding: "utf8" }
    );
    expect(out).toContain("PHASE7_MARKETPLACE_OK");
  });
});
